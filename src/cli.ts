...
async function main() {
  ...
  if (!action) {
    ...
    console.log('  emulate-device <preset> [w h]   Phone viewport: iphone14 | pixel7 | custom | off');
    console.log('  tap <x> <y>                     Real touch tap (phone-emulation mode)');
    console.log('  swipe <x1> <y1> <x2> <y2> [ms]  Touch swipe/drag gesture');
  ...
  try {
    switch (action) {
      ...
      case 'emulate-device': {
        const preset = args[0];
        if (!preset) { console.error('Usage: emulate-device <iphone14|pixel7|custom|off> [width height]'); process.exit(1); }
        const result = await command({ type: 'emulate_device', preset, width: args[1] ? Number(args[1]) : undefined, height: args[2] ? Number(args[2]) : undefined });
        console.log('Emulation:', JSON.stringify(result.result?.emulation ?? null));
        break;
      }
      ...
      case 'tap': {
        const x = Number(args[0]);
        const y = Number(args[1]);
        if (!Number.isFinite(x) || !Number.isFinite(y)) { console.error('Usage: tap <x> <y>'); process.exit(1); }
        await command({ type: 'tap', x, y });
        console.log(`Tapped: ${x},${y}`);
        break;
      }

      case 'swipe': {
        const [x1, y1, x2, y2, ms] = args.map(Number);
        if ([x1, y1, x2, y2].some((n) => !Number.isFinite(n))) { console.error('Usage: swipe <x1> <y1> <x2> <y2> [durationMs]'); process.exit(1); }
        await command({ type: 'swipe', x1, y1, x2, y2, durationMs: Number.isFinite(ms) ? ms : undefined });
