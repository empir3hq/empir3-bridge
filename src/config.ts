...
import { API_PROVIDER_SLUGS } from './api-provider-catalog.js';
import { readPersistentJson, writePersistentJson } from './persistent-json.js';
...
export interface BridgeConfig {
  ...
  // Per-provider permission to advertise the provider/model list to Empir3
  // and execute its calls locally. Keys never leave this device.
  apiProviderSharing: Record<string, boolean>;
}
...
function defaultConfig(): BridgeConfig {
  ...
  return {
    ...
    apiKeys: Object.fromEntries(API_PROVIDER_SLUGS.map(slug => [slug, ''])),
    apiProviderSharing: Object.fromEntries(API_PROVIDER_SLUGS.map(slug => [slug, false])),
  };
}
...
function hydrate(raw: Partial<BridgeConfig>): BridgeConfig {
  ...
  const rawSharing: any = (raw as any).apiProviderSharing || {};
  const apiProviderSharing: Record<string, boolean> = {};
  for (const slug of API_PROVIDER_SLUGS) {
    const saved = typeof rawKeys[slug] === 'string' ? rawKeys[slug] : '';
    apiKeys[slug] = slug === 'anthropic' && !saved ? legacyAnthropic : saved;
    // Older Bridge builds described these keys as Empir3 routes but had no
    // working sharing toggle. Preserve that intent during migration.
    apiProviderSharing[slug] = typeof rawSharing[slug] === 'boolean'
      ? rawSharing[slug]
      : apiKeys[slug].length > 0;
  }
  return {
...
export function loadConfig(): BridgeConfig {
  if (existsSync(CONFIG_FILE) || existsSync(CONFIG_BACKUP_FILE)) {
    const raw = readPersistentJson(CONFIG_FILE, {
      backupPath: CONFIG_BACKUP_FILE,
      defaultValue: defaultConfig,
      mode: 0o600,
      onRecovery: (message) => console.warn(`[Bridge config] ${message}`),
      validate: (value): value is Partial<BridgeConfig> => !!value && typeof value === 'object' && !Array.isArray(value),
      writeDefault: true,
    });
    return hydrate(raw);
  }
  if (!existsSync(LEGACY_CONFIG_FILE)) return defaultConfig();
  try { return hydrate(JSON.parse(readFileSync(LEGACY_CONFIG_FILE, 'utf-8'))); }
...
export function saveConfig(patch: Partial<BridgeConfig>): BridgeConfig {
  ...
  const mergedSharing = { ...current.apiProviderSharing, ...((patch as any).apiProviderSharing || {}) };
  const next: BridgeConfig = {
    ...current,
    ...patch,
    enabledTools: { ...current.enabledTools, ...(patch.enabledTools || {}) },
    apiKeys: mergedKeys,
    apiProviderSharing: mergedSharing,
  ...
  writePersistentJson(CONFIG_FILE, next, { backupPath: CONFIG_BACKUP_FILE, mode: 0o600 });
  return next;
}
...
export function publicConfig(c: BridgeConfig = loadConfig()) {
  return {
    ...
    apiKeysSet: Object.fromEntries(API_PROVIDER_SLUGS.map(slug => [slug, (c.apiKeys[slug] || '').length > 0])),
    apiProviderSharing: Object.fromEntries(API_PROVIDER_SLUGS.map(slug => [slug, c.apiProviderSharing[slug] === true])),
  };
}
