...
import { safeConversationId } from './local-resource-policy.mjs';
...
async function dispatchTool(name: string, input: any, bridgeBaseUrl: string): Promise<{ ok: boolean; output: string }> {
  ...
  try {
    switch (name) {
      ...
      case 'browser_click':        return ok(JSON.stringify(await post({ type: 'click', selector: input.selector }), null, 2));
      case 'browser_click_ref':    return ok(JSON.stringify(await post({ type: 'click_ref', ref: input.ref }), null, 2));
      case 'browser_click_xy':     return ok(JSON.stringify(await post({ type: 'click_xy', x: input.x, y: input.y }), null, 2));
      case 'desktop_click':        {
      ...
      case 'browser_type':         return ok(JSON.stringify(await post({ type: 'type', selector: input.selector, text: input.text }), null, 2));
      case 'browser_type_ref':     return ok(JSON.stringify(await post({ type: 'type_ref', ref: input.ref, text: input.text }), null, 2));
      case 'browser_press':        return ok(JSON.stringify(await post({ type: 'press', text: input.key }), null, 2));
      ...
      case 'browser_chat':         { await post({ type: 'chat', message: input.message }); return ok(`Sent to trusted Bridge chat: ${input.message}`); }
...
function persistTurn(conversationId: string, entry: any) {
  try {
    ...
    if (!safeId) return;
    ensureConvDir();
    appendFileSync(join(CONV_DIR, `${safeId}.jsonl`), JSON.stringify({ ...entry, ts: new Date().toISOString() }) + '\n');
...
export function readConversation(id: string): any[] {
  try {
    const safeId = safeConversationId(id);
    if (!safeId) return [];
    const path = join(CONV_DIR, `${safeId}.jsonl`);
...
export async function* streamChat(req: StreamChatRequest): AsyncGenerator<ChatEvent> {
  ...
  const conversationId = req.conversationId ? safeConversationId(req.conversationId) : randomUUID();
  if (!conversationId) { yield { type: 'error', message: 'Invalid conversation id' }; return; }
