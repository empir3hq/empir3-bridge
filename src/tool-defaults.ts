...
export type ToolGroup = 'read' | 'navigate' | 'interact' | 'desktop' | 'recordings' | 'eval' | 'advisor' | 'higgsfield' | 'providers' | 'capabilities' | 'clis';
...
export const TOOL_META: ToolMeta[] = [
  ...
  { name: 'bridge_setup_status', group: 'advisor', defaultEnabled: true,
    blurb: 'Report the first-use desktop setup checklist: trusted control surface, monitor detection, saved click calibration, and recording/playback readiness.' },
  ...
  { name: 'bridge_overlay_reinject', group: 'advisor', defaultEnabled: true,
    blurb: 'Compatibility security command. Removes any pre-0.3.72 page-world overlay and returns the trusted localhost control URL.' },
  ...
  { name: 'browser_audit_page', group: 'read', defaultEnabled: true,
    blurb: 'Inspect one settled page in a single call: full-page screenshot, compact accessibility facts, layout overflow, runtime errors, and failed resources. Prefer this over repeated scroll-and-screenshot loops when verifying a page.' },
  ...
  { name: 'browser_emulate_device', group: 'interact', defaultEnabled: false,
    blurb: 'Switch the tab into a phone viewport (iphone14 / pixel7 / custom / off): device metrics + touch + mobile UA. Click tools auto-route through touch taps while active. Chrome engine only — real Safari/WebKit rendering needs the Playwright harness.' },
  { name: 'browser_tap',        group: 'interact', defaultEnabled: false,
    blurb: 'Real touch tap at viewport CSS coordinates, auto-calibrated against browser zoom. The click for phone-emulation mode, where pages ignore mouse events.' },
  { name: 'browser_swipe',      group: 'interact', defaultEnabled: false,
    blurb: 'Touch swipe/drag between two viewport points. Scroll lists, dismiss bottom sheets, drag sliders while device emulation is active.' },
  ...
  { name: 'browser_run_checks', group: 'interact', defaultEnabled: false,
    blurb: 'Run a bounded checklist of at most 12 browser read/interact steps inside the Bridge and return compact receipts. Navigation and arbitrary JavaScript are intentionally excluded.' },
  ...
  { name: 'browser_accuracy_lab_sweep', group: 'desktop', defaultEnabled: false,
    blurb: 'Run the fixed 103-target Accuracy Lab with real OS clicks and retain a target-by-target proof receipt. Only this trusted sweep may promote click calibration.' },
  ...
  { name: 'desktop_toolbar', group: 'desktop', defaultEnabled: false,
    blurb: 'Open or close the movable desktop toolbar widget with focus, release, trusted local chat, recording, playback, and monitor-local quick calibration controls.' },
  ...
  { name: 'browser_chat',         group: 'recordings', defaultEnabled: false,
    blurb: 'Push a message into the trusted localhost Bridge chat. Use to surface progress or questions without exposing replies to the webpage.' },
  { name: 'browser_read_chat',    group: 'recordings', defaultEnabled: false,
    blurb: 'Read recent messages from the trusted localhost Bridge chat (both user and agent).' },
  ...
  // ── Capability rail (Whisper / Kokoro / A1111 / ComfyUI) ────
  // Hidden when no non-chat provider exists. Provider sharing remains the
  // per-endpoint consent boundary; this is the local dispatcher kill switch.
  { name: 'custom_capability', group: 'capabilities', defaultEnabled: false,
    blurb: 'Run a shared local speech-to-text, text-to-speech, or image-generation endpoint configured on this Bridge. Spends this computer\'s compute or endpoint quota.' },
];
