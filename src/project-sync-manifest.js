        ...
        const discovered = await Promise.all(batch.map(async (entry) => {
          ...
          try {
            ...
            if (fileStat.size > maxFileBytes) {
              ...
              // Still a REAL local file — it stays in the manifest, flagged,
              // so (a) the sync reconciler never reads "excluded by size" as
              // "user deleted it" and reverse-deletes the server's canonical
              // copy (2026-08-07: every server-generated video > the frame
              // cap was phantom-trashed one scan tick after landing — 25
              // files over 9 days, runway-turn.mp4 caught live), and (b) the
              // server sees it exists and skips redundant re-pushes. The
              // `oversize` flag tells the upload loop to skip content sync.
              return {
                kind: 'file',
                file: {
                  path: relativePath,
