...
import { shouldRetryScreenshotCdpFailure } from './browser-request-policy.js';
...
async function resolveBrowserActionTarget(selector: string, label: string, requireEditable = false): Promise<BrowserActionTarget> {
  const resultText = await cdpEvaluate(`(function() {
    const selector = ${JSON.stringify(selector)};
    const label = ${JSON.stringify(label)};
    const el = document.querySelector(selector);
    if (!el) return JSON.stringify({ error: 'Element not found: ' + label });
    (el.scrollIntoViewIfNeeded ? el.scrollIntoViewIfNeeded() : el.scrollIntoView({ block: 'center', inline: 'center' }));
    const style = getComputedStyle(el);
    const r = el.getBoundingClientRect();
    if (r.width <= 0 || r.height <= 0 || style.display === 'none' || style.visibility === 'hidden' || style.pointerEvents === 'none') {
      return JSON.stringify({ error: 'Element is not actionable: ' + label });
    }
    if (el.disabled === true || el.getAttribute('aria-disabled') === 'true') {
      return JSON.stringify({ error: 'Element is disabled: ' + label });
    }
    const x = r.left + r.width / 2;
    const y = r.top + r.height / 2;
    const hit = document.elementFromPoint(x, y);
    if (!hit || !(hit === el || el.contains(hit))) {
      return JSON.stringify({ error: 'Element is obscured at its center: ' + label });
    }
    const editable = el.isContentEditable === true || el.tagName === 'INPUT' || el.tagName === 'TEXTAREA';
    if (${requireEditable ? 'true' : 'false'} && !editable) {
      return JSON.stringify({ error: 'Element is not editable: ' + label });
    }
    return JSON.stringify({ x, y, tag: el.tagName.toLowerCase(), editable });
  })()`);
...
async function clickBrowserTarget(selector: string, label: string): Promise<any> {
  ...
  const token = `e3-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  await cdpEvaluate(`(function() {
    ...
    const handler = function(event) {
      if (event.isTrusted && (event.target === el || el.contains(event.target))) events.push(event.type);
    };
    const types = ['pointerdown', 'mousedown', 'touchstart', 'click'];
    types.forEach(type => el.addEventListener(type, handler, true));
    store[token] = { el, events, handler, types };
    return true;
  })()`);
  await clickByXY(target.x, target.y);
  await sleep(40);
  const receiptText = await cdpEvaluate(`(function() {
    const token = ${JSON.stringify(token)};
    const store = window.__empir3ActionReceipts || {};
    const receipt = store[token];
    if (!receipt) return JSON.stringify([]);
    receipt.types.forEach(type => receipt.el.removeEventListener(type, receipt.handler, true));
    delete store[token];
    return JSON.stringify(receipt.events);
  })()`);
  const events = JSON.parse(receiptText || '[]');
  if (!Array.isArray(events) || events.length === 0) {
    throw new Error(`Browser dispatched a click but ${label} received no trusted pointer event`);
  }
...
async function typeIntoBrowserTarget(selector: string, label: string, text: string): Promise<any> {
  const target = await resolveBrowserActionTarget(selector, label, true);
  const originalText = await cdpEvaluate(`(function() {
    const el = document.querySelector(${JSON.stringify(selector)});
    if (!el) return JSON.stringify({ error: 'not_found' });
    const value = el.isContentEditable ? (el.textContent || '') : String(el.value ?? '');
    return JSON.stringify({ value });
  })()`);
  const original = JSON.parse(originalText || '{}');
  if (original.error) throw new Error(`Element disappeared before typing: ${label}`);

  try {
    await clickBrowserTarget(selector, label);
    const focusText = await cdpEvaluate(`(function() {
      const el = document.querySelector(${JSON.stringify(selector)});
      const active = document.activeElement;
      return String(!!el && !!active && (active === el || el.contains(active)));
    })()`);
    if (focusText !== 'true') throw new Error(`Click did not focus editable element: ${label}`);
    ...
    if (observed.error) throw new Error(`Element disappeared while typing: ${label}`);
    if (observed.value !== text) {
      throw new Error(`Typing verification failed for ${label}: expected ${text.length} characters, observed ${String(observed.value || '').length}`);
    }
  } catch (error) {
...
let emulationSession: { send: (m: string, p?: any, t?: number) => Promise<any>; close: () => void } | null = null;

async function setDeviceEmulation(preset: string, width?: number, height?: number): Promise<any> {
  ...
  return { emulation: deviceEmulation };
}
...
async function openPageCdpSession(): Promise<{ send: (method: string, params?: any, timeoutMs?: number) => Promise<any>; close: () => void }> {
  const target = await currentPageTarget();
  const ws = new WebSocket(target.webSocketDebuggerUrl);
  ...
  const send = (method: string, params: any = {}, timeoutMs = CDP_COMMAND_TIMEOUT_MS) => new Promise<any>((resolve, reject) => {
    const id = cdpId++;
    const timer = setTimeout(() => { pending.delete(id); reject(new Error(`CDP gesture timeout: ${method}`)); }, timeoutMs);
...
async function pressKey(key: string): Promise<void> {
  ...
  if (key.includes('+')) {
    ...
    const modifiers = modifier === 'control' || modifier === 'ctrl' ? 2 :
                      modifier === 'shift' ? 8 :
                      modifier === 'alt' ? 1 : 0;
    const named = keyMap[mainKey];
    const virtualKeyCode = named?.keyCode || (mainKey.length === 1 ? mainKey.toUpperCase().charCodeAt(0) : 0);
    const code = named?.code || (mainKey.length === 1 ? `Key${mainKey.toUpperCase()}` : mainKey);

    await cdpSend('Input.dispatchKeyEvent', {
      type: 'keyDown',
      key: named?.key || mainKey,
      code,
      modifiers,
  ...
  try {
    ...
    if (method === 'POST') {
      ...
      if (path === '/navigate') {
        ...
        sendJSON(res, { title, url: currentUrl, verified: true });
        return;
      }
      ...
      if (path === '/action') {
        ...
        switch (kind) {
          case 'click':
            if (body.ref) {
              sendJSON(res, await clickByRef(String(body.ref)));
            } else if (body.selector) {
              sendJSON(res, await clickBrowserTarget(String(body.selector), `selector:${body.selector}`));
            } else if (typeof body.x === 'number' && typeof body.y === 'number') {
              await clickByXY(body.x, body.y);
              sendJSON(res, { success: true, dispatched: true, verified: false, coordinates: { x: body.x, y: body.y } });
            } else {
          ...
          case 'type': {
            ...
            if (typeTarget) {
              sendJSON(res, await typeIntoBrowserTarget(
                typeTarget,
              ...
              sendJSON(res, { success: true, dispatched: true, verified: false, target: 'focused element' });
            }
            break;
          }
          ...
          case 'press':
            ...
            sendJSON(res, { success: true, dispatched: true, verified: false, key: body.key || body.text || '' });
            break;
          ...
          case 'emulate_device': {
            const result = await setDeviceEmulation(String(body.preset || 'off'), body.width, body.height);
