  ...
  async () => {
    try {
      ...
      const compact = {
        ...
        pageCount: Array.isArray(status?.pages) ? status.pages.length : undefined,
...
server.tool(
  ...
  async ({ preset, width, height }) => {
    const result = await bridgeCommand({ type: 'emulate_device', preset, width, height });
    return jsonResult(result);
  }
);
