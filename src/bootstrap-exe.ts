 ...
 *   1. process.env.EMPIR3_DESKTOP_EXE    (set by the Electron shell)
 *   2. process.env.EMPIR3_BOOTSTRAP_EXE  (set by the Go stub for its Node child)
 *   3. %APPDATA%/Empir3/bridge-bootstrap.json → bootstrapPath
 *   4. stable %APPDATA%/Empir3/Empir3Setup.exe (if present)
 *   5. process.execPath — ONLY for a genuine old Node-SEA process (isSea()),
...
export function resolveBootstrapExe(): string | null {
  const fromDesktop = (process.env.EMPIR3_DESKTOP_EXE || '').trim();
  if (fromDesktop && existsSync(fromDesktop)) return fromDesktop;
