...
import { deliverAsset } from '../asset-transport.js';
...
function spawnCapture(bin: string, argv: string[], timeoutMs: number): Promise<SpawnResult> {
  return new Promise(resolve => {
    ...
    let child;
    try {
      const invocation = resolveHiggsfieldSpawn(bin, argv);
      if (!invocation) throw new Error('unsupported Higgsfield Windows command shim — reinstall @higgsfield/cli');
      child = registerOwnedCliProcess(spawn(invocation.command, invocation.args, { windowsHide: true }), 'higgsfield-cli');
    } catch (e: any) {
    ...
    const timer = setTimeout(() => {
      timedOut = true;
      killed = true;
      void terminateCliProcessTree(child, { signal: 'SIGTERM', reason: 'Higgsfield CLI timeout' });
      setTimeout(() => { void terminateCliProcessTree(child, { signal: 'SIGKILL', reason: 'Higgsfield CLI timeout force' }); }, SIGTERM_GRACE_MS);
    }, timeoutMs);
...
/** Resolve the npm Windows shim without invoking cmd.exe. Passing a user
 * prompt through `cmd /c` makes shell metacharacters such as `&` executable;
 * production prompts containing "HOME & HAND" were split into bogus commands.
 * The npm shim is only a thin Node launcher, so call its package entrypoint
 * with this bridge's Node runtime and keep every prompt as an argv value. */
export function resolveHiggsfieldSpawn(
  bin: string,
  argv: string[],
  ...
  const entry = join(dirname(bin), 'node_modules', '@higgsfield', 'cli', 'bin', 'higgsfield.js');
  if (!existsSync(entry)) return null;
  return { command: nodeBin, args: [entry, ...argv] };
}
...
export async function higgsfieldStatus(_params: Record<string, unknown> = {}): Promise<{ success: true; result: HiggsfieldStatusResult }> {
  ...
  if (version.exitCode !== 0 || !m) {
    return {
      ...
      result: {
        ...
        version: null,
        authenticated: false,
        error: (version.stderr || version.stdout || 'Higgsfield CLI binary is incomplete; reinstall it.').trim().slice(0, 500),
      },
    };
  }
  ...
  const authed = tok.exitCode === 0 && /^(?:hf_|oat_)\S+\s*$/.test(tok.stdout.trim());
  return {
    success: true,
    result: {
      installed: true,
...
export async function handleHiggsfieldCliCommand(
  ...
  // A handler returning a function/undefined/malformed envelope — or a bare
  // failure with no stage/error — is a Bridge bug, not a provider failure.
  // 0.3.84 shipped the generate closure uninvoked and this exact spot
  // collapsed it into a generic "generate_failed" that masked the outage for
  // hours. Tag it loudly and log the full envelope server-side.
  const malformedResult = !out || typeof out !== 'object' || typeof out.success !== 'boolean'
    || (out.success !== true && out.stage === undefined && out.error === undefined);
  if (malformedResult) {
    let preview: string;
    try { preview = JSON.stringify(out)?.slice(0, 500) ?? String(out).slice(0, 500); } catch { preview = String(out).slice(0, 500); }
    console.error(`[higgsfield] internal_bad_result: typeof=${typeof out} envelope=${preview}`);
    send('higgsfield:cli:error', {
      id,
      stage: 'internal_bad_result',
      error: `higgsfield handler returned a malformed result (typeof=${typeof out}) — a Bridge bug, not a provider failure`,
    });
    return;
  }
  if (out.success !== true) {
  ...
  const mimeType = mimeForArtifact(artifactPath, kind);
  try {
    const delivered = await deliverAsset(bytes, mimeType, payload?.upload);
    ...
    send('higgsfield:cli:error', { id, stage: 'upload', error: e?.message || String(e) });
  }
}
