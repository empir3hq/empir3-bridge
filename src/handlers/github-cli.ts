  ...
  return new Promise(resolve => {
    ...
    try {
      ...
      if (isWinShim) {
        ...
        // is passed literally.
        child = registerOwnedCliProcess(spawn('cmd.exe', ['/d', '/s', '/c', bin, ...argv], { windowsHide: true }), 'github-cli');
      } else {
        child = registerOwnedCliProcess(spawn(bin, argv, { windowsHide: true }), 'github-cli');
      }
    } catch (e: any) {
    ...
    const timer = setTimeout(() => {
      timedOut = true;
      killed = true;
      void terminateCliProcessTree(child, { signal: 'SIGTERM', reason: 'GitHub CLI timeout' });
      setTimeout(() => { void terminateCliProcessTree(child, { signal: 'SIGKILL', reason: 'GitHub CLI timeout force' }); }, SIGTERM_GRACE_MS);
    }, timeoutMs);
