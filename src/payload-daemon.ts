...
const { configureAccountProfile } = require('./account-profile.js');
...
function chooseCdpPort(preferred: number, bridgePort: number, wrapperPort: number): number {
  if (process.platform !== 'win32') return preferred;
  const holders = listenerPids(preferred);
  // Existing Bridge Node/Chrome processes are reaped below. Only relocate for
  // an unrelated owner (the observed Stoic failure was Fifine/Genie on 9222).
  const unrelated = holders.some((pid) => !['node.exe', 'chrome.exe'].includes(processName(pid)));
  if (!unrelated) return preferred;
  for (let offset = 1; offset <= 100; offset++) {
    const candidate = preferred + offset;
    if (candidate > 65535 || candidate === bridgePort || candidate === wrapperPort) continue;
    if (listenerPids(candidate).length === 0) {
      console.log(`[empir3-bridge] CDP port ${preferred} belongs to another app; selected ${candidate}`);
      return candidate;
    }
  }
  throw new Error(`Chrome CDP port ${preferred} belongs to another app and no fallback port is available`);
}
...
export async function start() {
  ...
  const requestedCdpPort = Number(process.env.EMPIR3_CDP_PORT || process.env.CDP_PORT || 9222);
  const cdpPort = chooseCdpPort(requestedCdpPort, bridgePort, wrapperPort);
  ...
  process.env.EMPIR3_CDP_PORT = String(cdpPort);
  ...
  const profile = configureAccountProfile({
    log: (message: string) => console.log(`[empir3-bridge] ${message}`),
  });
  console.log(`[empir3-bridge] controlled-browser profile=${profile.paired ? 'account-scoped' : (profile.explicit ? 'explicit' : 'unpaired')}`);
