...
function agyAuthSignal(): { authenticated: boolean; via: 'keyring' | 'creds_file' | 'install_marker' | 'env' | 'none' } {
  ...
  // on first run, AFTER keyring provisioning. If it's there AND the agy
  // binary is installed, the chain auth almost certainly hits.
  ...
  if (existsSync(installMarker)) {
    ...
    if (process.platform === 'win32') {
      try {
        ...
        if (result.status === 0 && /gemini:antigravity/i.test(result.stdout || '')) {
          return { authenticated: true, via: 'keyring' };
        }
      } catch { /* cmdkey unavailable — fall through */ }
      // cmdkey returned no match — treat install marker as a weak yes.
...
const CLI_RUN_MODELS: Record<string, { label: string; lend: () => boolean; find: () => Promise<string | null> }> = {
  ...
  gemini: { label: 'Google Gemini', lend: geminiDeviceOptedIn, find: () => findPreferredExecutable('gemini') },
...
async function cliRun(cmd: any): Promise<any> {
  ...
  const runOnce = async (): Promise<CliRunRecord> => {
    ...
    const r = await runCliCapture(normalizeCliShim(bin), argv, { cwd, env, stdin, timeoutMs });
...
async function cliRunRoster(): Promise<any> {
  const authByModel: Record<string, () => boolean> = {
    ...
    gemini: () => geminiAuthSignal().authenticated,
...
async function runClaudeCliSee(payload: any, emit: (type: string, payload: any) => void) {
  return runCliSee(payload, emit, {
    ...
    buildInvocation: async ({ prompt, system, imageBase64, mimeType, model, tempDir }) => {
      ...
      // sent via stream-json stdin (verified 2.1.31). The `@<path>` prompt
      // syntax works — write the image to a temp file and reference it.
      // Critical: `@<path>` must be on the FIRST line of the positional
      // prompt, before any newlines. If you put `<system>\n\n@image.png`,
      // the CLI's @-reference parser skips the path entirely and the
      // model replies "I need to see an image" because it never got one.
      // (Verified 2026-05-28 by isolating each variable.) Pass system via
      // `--system-prompt` flag so the positional stays clean.
      const fsp = await import('fs/promises');
...
async function runCodexCliSee(payload: any, emit: (type: string, payload: any) => void) {
  return runCliSee(payload, emit, {
    ...
    buildInvocation: async ({ prompt, system, imageBase64, mimeType, model, tempDir }) => {
      ...
      if (model) args.push('--model', model);
      args.push(fullPrompt, '--image', imagePath);
      return {
        args,
        cwd: tempDir,
...
async function runAgyCliSee(payload: any, emit: (type: string, payload: any) => void) {
  // Same @<path> first-line syntax as Gemini (agy is the Gemini-CLI
  // successor; same parser ancestry). System intent is folded after the
  // @ ref + user prompt because agy has no documented --system-prompt
  // flag — verified via `agy --help` after first install.
  return runCliSee(payload, emit, {
    cliName: 'agy',
    wirePrefix: 'agy',
    optedInCheck: () => agyDeviceOptedIn(),
    optedOutMessage: 'Device owner has not opted in. Enable "Lend Empir3 my Antigravity CLI" in the bridge tray before routing vision through this PC.',
    buildInvocation: async ({ prompt, system, imageBase64, mimeType, model, tempDir }) => {
      const fsp = await import('fs/promises');
      const ext = (mimeType.split('/')[1] || 'png').replace(/[^a-z0-9]/gi, '');
      const imageName = `image.${ext}`;
      ...
      // forwarded by the server (it's part of the wire shape), just not
      // consumed by the spawn today.
...
async function desktopScreenshotZoom(params: any): Promise<any> {
  ...
  return { ...shot, center: { x: cx, y: cy }, radius: r, cropOrigin: { x: cx - r, y: cy - r } };
}
