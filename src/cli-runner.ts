...
export async function* streamCli(req: CliStreamRequest): AsyncGenerator<StreamEvent> {
  ...
  registerOwnedCliProcess(child, 'claude-chat');

  // Hook abort signal — SIGTERM, then SIGKILL after grace period.
  const abortHandler = () => {
    void terminateCliProcessTree(child, { signal: 'SIGTERM', reason: 'Claude chat abort' });
    setTimeout(() => {
      void terminateCliProcessTree(child, { signal: 'SIGKILL', reason: 'Claude chat abort force' });
    }, SIGTERM_GRACE_MS);
  };
  if (req.signal) {
