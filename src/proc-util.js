...
/**
 * Parse the PIDs listening on a requested set of TCP ports from Windows
 * `netstat -ano -p tcp` output.
 *
 * Keeping this parser pure makes the safety boundary directly testable. The
 * local address may be IPv4 (`127.0.0.1:3006`) or IPv6 (`[::]:3006`), so the
 * final colon-delimited component is the only port value we trust.
 */
function parseWindowsListeningPids(raw, ports) {
  ...
  const pids = new Set();
  if (requested.size === 0 || typeof raw !== 'string') return pids;
  ...
  for (const line of raw.split(/\r?\n/)) {
    ...
    if (!portMatch || !requested.has(portMatch[1])) continue;
...
/**
 * Return the Windows PIDs proven to own one of the requested listening ports.
 * `null` means the lookup itself failed; callers must fail closed rather than
 * widen the reap to every Bridge-looking process on the machine.
 */
function windowsListeningPids(ports) {
  ...
  try {
    ...
    return parseWindowsListeningPids(raw, ports);
  } catch {
    return null;
  }
}
 ...
 * the ports actually live, and by resolving the exact Windows listener PIDs.
 * Windows fails closed if its listener lookup is unavailable: an honest
 * EADDRINUSE is safer than killing the installed payload, MCP servers, or CLI
 * children merely because their command lines live under `.empir3-bridge`.
 */
function listOwnedProcesses({
  ...
  const out = [];
  const rows = processRows || psSnapshot();
  const requestedPorts = [pwPort, bridgePort].filter(Boolean);
  const provenWindowsOwners = platform === 'win32' && requestedPorts.length
    ? (windowsPortOwners === undefined ? windowsListeningPids(requestedPorts) : windowsPortOwners)
    : null;
  ...
  for (const { pid, cmd } of rows) {
    ...
    if (kind !== 'chrome' && (pwPort || bridgePort)) {
      const env = environmentForPid ? environmentForPid(pid) : readProcEnviron(pid);
      ...
      if (theirPw || theirBridge) {
        ...
        // Windows has no /proc environment. Require positive proof that this
        // exact PID owns one of this launcher's requested ports. A failed
        // lookup is represented by null and therefore matches nothing.
        if (!provenWindowsOwners || !provenWindowsOwners.has(pid)) continue;
      }
    }
