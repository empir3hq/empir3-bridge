...
const { revokePriorDeviceCredential } = require('./account-handover.js');
...
async function enrollIfNeeded(opts = {}) {
  ...
  const handover = await revokePriorDeviceCredential(nextAuth);
  if (handover.attempted && !handover.revoked) {
    log(`warning: prior device credential could not be retired during account switch (HTTP ${handover.status || 'unavailable'})`);
  }
  writeAuthFileSecurely(nextAuth);
