...
- Chrome launcher hand-offs no longer latch a still-running Windows browser as
  closed. Reachable CDP pages are now the browser-liveness authority, stale
  close state self-heals, and status reports browser liveness explicitly.
- Browser navigation now succeeds only after the active page URL is verified;
  target-list history can no longer turn a failed navigation into success.

## [0.3.102] - 2026-09-03

### Fixed

- Closing the final controlled Chrome tab now treats the browser process exit
  as positive completion evidence instead of returning a CDP connection error.

## [0.3.101] - 2026-09-03

### Fixed

- Managed Linux Agent Boxes can request a signed update directly from the
  Bridge console through a narrowly scoped systemd path unit instead of being
  blocked by the desktop tray requirement.
- Agent browser close now closes the exact owned Chrome tab and reports success
  only after Chrome confirms the target disappeared.
- Linux Chrome startup removes stale profile singleton locks only when their
  recorded owner PID is proven dead, recovering safely after hard shutdowns.
- Production release indexes reject health labels that deployed updaters do
  not understand, preventing older Bridges from becoming stranded on a valid
  release.
...
- Browser verification can run bounded multi-step checks in one trusted Bridge
  request, returning compact per-step receipts instead of forcing an agent to
  resend full page state after every click, type, wait, and assertion.
- The Accuracy Lab now verifies its 103-target run from trusted click receipts
  and final statistics, so a zeroed or fabricated result cannot pass.

### Changed

- Browser guidance now prefers structured text and snapshots for interaction,
  reserving screenshots for visual evidence and reducing repeated vision work.

## [0.3.99] - 2026-08-29

### Fixed

- Account re-pairing no longer makes an established Bridge-controlled browser
  profile look silently erased. When the newly selected account profile is
  materially less established, the local Account screen explains the secure
  separation and offers an explicit restore or keep-separate choice.
- Confirmed profile recovery copies only from another Bridge-owned account
  profile, verifies the staged file count and byte count, preserves the source,
  backs up the current target, and applies before controlled Chrome restarts.
  The user's normal Chrome profile remains outside the lookup and copy paths.

## [0.3.98] - 2026-08-29

### Fixed

- Long custom-provider turns emit correlated proof-of-life while waiting for a
  non-streaming provider response. The app adapter treats that event as stream
  activity without fabricating text, and the Bridge honors the absolute
  provider timeout supplied by the app's admin runtime dial.
- Synthetic wheel input now follows the element under the viewport point and
  chains residual movement through scrollable ancestors to the document.
  `overflow:hidden` and `overflow:clip` residual ranges no longer trap a wheel
  intended for the page.
- Windows CLI updates quote space-bearing npm shim paths correctly,
  Antigravity background catalog probes run only when AGY is lent, shell
  execution shares the server's 300-second ceiling, and discovered provider
  models override stale catalog defaults.
- Windows launcher cleanup now reaps a Bridge process only when that exact PID
  owns one of the requested Bridge ports. Parallel source/test launches no
  longer sweep the installed payload, MCP servers, or CLI children merely
  because their command lines live under `.empir3-bridge`.

## [0.3.97] - 2026-08-28

### Fixed

- Codex, Gemini, Grok, and Antigravity project-work turns now receive a
  mandatory tool-execution contract in both the turn prompt and the MCP server
  instructions, preventing an attached toolkit from silently degrading into a
  prose-only build response.
- Antigravity 1.1.22 now receives its isolated MCP catalog at
  `.gemini/config/mcp_config.json`; the previous `.antigravity/settings.json`
  path is ignored by current AGY and left lent turns without Empir3 tools.
- Antigravity's current `.gemini` OAuth and account receipts are copied into
  each isolated turn home, preserving an already-authorized Vault session
  without copying plugins, history, skills, or machine-global MCP catalogs.
- Added live Codex and Antigravity MCP smokes that prove `gpt-5.6-sol` and
  `gemini-3.7-flash-high` call the attached Empir3 `Write` tool with exact
  arguments on Windows; Codex is also proven on the maintained Vault host.
- The packaged scale-browser release smoke uses headless Chrome only on hosted
  CI workers, avoiding a Windows runner desktop-session race while retaining
  real Chrome/CDP health acceptance and leaving installed Bridge behavior
  unchanged.
...
- Installed CLI lifecycle actions now return visible Windows and POSIX terminal
  receipts for update, authentication, installation, and sign-out, including a
  redacted diagnostic tail when a command fails.
- Windows can issue a narrowly scoped wake nudge and keep-awake request before
  a controlled-browser action while still refusing honestly at the secure
  desktop boundary.

### Fixed

- Controlled-browser profiles are now isolated by Empir3 account so signing
  into a second account cannot inherit the first account's browser sessions.
- Re-pairing the Bridge to a different Empir3 account retires the prior device
  credential before the new account becomes active.
- Antigravity model discovery accepts the CLI's tab-separated catalog output
  instead of incorrectly reporting no available models.
- z.ai Coding Plan credentials use their dedicated endpoint and remain
  separate from standard z.ai API credentials.
- Codex tool turns retain the private Empir3 MCP server injection and refuse to
  continue if the toolkit cannot attach.

## [0.3.95] - 2026-08-27

### Fixed

- Project mirrors now remember the last content synchronized in either
  direction, allowing routine workspace updates to replace an unchanged local
  file instead of being misclassified as a two-writer conflict.
- Genuine local edits park one server-conflict sidecar per incoming content
  version; reconnects no longer create duplicate sidecars.
- Sync state files, their temporary flush files, and conflict sidecars are
  excluded from manifests, uploads, hydration, and deletion reconciliation so
  they cannot propagate to other mirrors.
- Server upload acknowledgements record the accepted local hash, preserving the
  correct three-way-sync base for the next workspace update.
- Refreshed Grok credentials now retry transient Windows destination locks
  during atomic replacement, preventing a successful CLI token rotation from
  leaving the Bridge's durable session on the revoked prior token.

## [0.3.94] - 2026-08-21

### Fixed

- Payload-retention discovery now interprets Windows payload paths correctly
  even when the Bridge test or package runs on Linux or macOS.
- Packaged desktop smoke tests create every isolated writable profile and
  temporary directory before launch, allowing the text-mode CLI isolation
  path to create its per-run workspace on Linux.

## [0.3.93] - 2026-08-21

### Fixed

- Codex text, tool, and vision turns now always use `--ignore-user-config`,
  `--ignore-rules`, ephemeral sessions, and a private temporary cwd. OAuth
  remains in `CODEX_HOME`, while the owner's `config.toml`, MCP servers,
  hooks, profiles, and project instructions cannot enter a lent turn. Tool
  output is held until the private Empir3 MCP server lists successfully; one
  clean retry is allowed, then the turn fails honestly instead of running
  toolless.
- Antigravity and Gemini now use auth-only temporary homes for both tool and
  no-tool turns. AGY also disables slash-command/skill expansion. Known
  account receipts are copied when a platform stores them on disk; global
  MCP catalogs, extensions/plugins, hooks, skills, settings, history, and
  project state are excluded. Gemini vision uses the same boundary.
- PTY-backed CLIs now run their no-tool isolation setup just like plain CLIs,
  closing the gap that previously left AGY text and vision turns in the
  owner's real home. Held 0.3.92 was rejected before installation after this
  cross-provider audit and was never published.
- The Bridge's direct `cli_run` route, Grok's live Verify action, and
  authenticated Codex/Grok/AGY model-catalog refreshes now use the same
  isolation boundary. Text-only direct runs use a private cwd; approved
  agentic runs retain their selected workspace while excluding personal MCPs,
  rules, skills, hooks, plugins, and persistent CLI sessions. Grok keeps its
  native workspace tools only for that explicitly approved agentic mode.
- Subscription-backed Claude, Codex, Gemini, AGY, and Grok launches scrub
  inherited vendor API credentials so a tray/service environment cannot
  silently replace the device owner's CLI account with metered API billing.

## [0.3.92] - 2026-08-21

### Fixed

- Claude lent turns no longer use safe mode, which also disabled the explicit
  per-turn Empir3 MCP server and made tool-bearing Team Tier turns report
  `mcp_servers: []`. They now preserve the owner's OAuth login while loading
  no user/project settings or skills, accepting MCP only from the strict
  per-turn config, and running from an empty temporary directory. The same
  isolation applies to Claude vision calls.
- Grok isolation now disables its Claude and Cursor compatibility import
  surfaces in both the private turn config and environment. Grok 1.0.5 could
  otherwise import those MCP catalogs even with an isolated `GROK_HOME`,
  causing local Verify to launch Higgsfield, codebase-memory, and Codex helper
  processes. Held 0.3.91 exposed both gaps and was never published.

## [0.3.91] - 2026-08-21

### Fixed

- Grok isolation now sets the CLI's dedicated `GROK_HOME` override to the
  per-turn auth-only directory. On Windows, changing `HOME`/`USERPROFILE`
  alone did not stop Grok 1.0.5 from loading the owner's real MCP catalog, so
  a local manual Verify could still start Higgsfield, codebase-memory, and
  Codex helpers even with `--tools ""`; the Vault did not expose the gap
  because that account has no personal Grok MCP catalog.

## [0.3.90] - 2026-08-21

### Fixed

- Lent Claude turns now preserve the owner's OAuth entitlement while forcing
  Claude safe mode and strict per-turn MCP configuration, so user/project
  hooks, instructions, plugins, skills, and unrelated MCP servers cannot leak
  into Empir3 turns or inflate their prompt cache.
- Grok's manual Verify action is single-flight and uses the same tool-free
  turn setup as relay turns, preventing repeated clicks from spawning multiple
  probes. A Windows-specific config-root leak remained in this held canary and
  is fixed forward in 0.3.91.
- Grok relay turns remove the CLI's native filesystem and shell tools while
  retaining lazy Empir3 MCP discovery, preventing a failed attach from writing
  deliverables to the Bridge PC and falsely reporting them as project files.
- Browser selector/ref clicks require a trusted pointer-event receipt, and
  targeted typing now focuses with a real click, sends keyboard input, and
  verifies the resulting value; MCP/chat callers receive those receipts.
- Systemd installs no longer force `EMPIR3_HEADLESS`/`BRIDGE_HEADLESS`, so an
  X11/Xvfb-equipped agent box is discovered as a drivable workstation.
- Antigravity prompt-file references now resolve relative to the exact CLI cwd
  instead of an unrelated system temp directory.
- Windows Bridge startup selects a free internal Chrome CDP port when another
  application owns 9222, without moving the public wrapper/Bridge ports.
- Claude updates on Windows now require a matching native npm package, run the
  vendor postinstall wiring, verify the resulting CLI, and roll back plus
  re-verify the previous version on failure.
- The lent-transcript retention sweep now also covers the `cli-see` root:
  every Claude `:see`/caption call spawns with a fresh temp dir as its cwd,
  so Claude Code left one single-transcript project directory behind per
  caption (observed live: 27 in one day on the Vault). Those dirs now age
  out with everything else, and any swept directory the sweep empties is
  pruned so the directory count itself stays bounded.

## [0.3.89] - 2026-08-18

### Fixed

- **Grok no longer hard-fails healthy answers that never needed a tool.**
  Grok 1.0.5 became a lazy MCP client: `tools/list` is fetched only when the
  model first wants a tool (verified live on the shared admin bridge — a
  tool-forcing prompt attaches and executes 3/3; a no-tool prompt exits
  toolless with a complete answer). The eager-attach verification, calibrated
  on an older build that probed at +4.3s pre-output, was classifying every
  no-tool answer as a toolless failure: two burned CLI runs, a hard
  `mcp_attach` error, and a hosted-API failover per prompt — plus a
  120-second output hold on long turns before the valve opened
  (`no tools/list within 120000ms` in bridge.log). Grok is now opted out of
  attach verification under the same E-005 doctrine that already exempts
  agy; gemini remains verified (still probed eager). Re-probe attach timing
  before ever re-enabling verification for a CLI.

## [0.3.88] - 2026-08-18

### Fixed

- **Grok relay turns can no longer hold a concurrency channel to the timeout
  wall after the work is done.** A relay turn's channel is released only when
  the CLI child exits, and three paths let a finished-or-abandoned child keep
  running for the full `timeout_sec` (operators saw channels pinned ~20-30
  minutes while the answer had streamed long before):
  - **No-tools turns now get the same per-turn isolation as tool turns**
    (isolated home + `--always-approve` + `--leader-socket`, empty MCP
    config). They previously ran in the operator's real `~/.grok` with no
    flags — measured locally, those runs intermittently hang after the final
    answer (1 in 6; isolated runs exit ~2s after output, 12/12) — and also
    bypassed the cross-tenant boundary and the refresh gate's rotation
    write-back, leaving the single-use refresh token exposed to the exact
    double-spend the 0.3.86 gate was built to prevent.
  - **A pending MCP `tools/call` can no longer outlive its turn.** The shim
    holds the CLI's HTTP response open with heartbeats (defeating the
    client's own timeout), so an orphaned call — superseded turn, hung
    invocable, dropped relay result — parked the child for the full 20-minute
    default. The wait is now capped just under the turn's own wall and fails
    back into the model so it can wrap up and exit.
  - **An abort that lands while the turn has no live child now sticks.**
    Previously it no-opped (mid shim/config setup or during the attach-retry
    respawn gap) and the child that spawned moments later ran an unattended
    turn to the wall. The turn id is marked and the child is terminated on
    arrival; markers expire after 5 minutes.

### Added

- **Channel-hold diagnostics in bridge.log.** Turn wall-hits were silent;
  now they log duration, time since the last stdout, and how many tool calls
  were still pending at the kill (also emitted on the wire as
  `since_last_output_ms` / `pending_tool_calls`). A `tools/call` pending
  more than 2 minutes warns while it waits, an orphaned call is named when
  it's failed back, and a `tool:result` that arrives for an unknown turn or
  call id is logged instead of silently dropped.

## [0.3.87] - 2026-08-16

### Fixed

- **The Grok auth classifier now catches the real revocation signature.**
  Minutes after 0.3.86 shipped, the live acceptance run on the revoked local
  session showed the CLI failing with
  `OIDC: token refresh HTTP error http_status=400 oauth2_error=Some("invalid_grant")`
  — and no existing pattern matched it (the words arrive as "token refresh",
  not "refresh token"), so the authorization badge stayed green and the new
  refresh gate's reactive latch never fired. `invalid_grant` now classifies as
  `refresh_rejected` and generic token-refresh errors as `refresh_failed`;
  rate-limit-flavored refresh errors stay unclassified per the billing guard.
- The refresh-boundary stress harness reads the nested `/api/command`
  cli_run envelope, so per-run error text and rejection classification are
  reported correctly.
...
- **The Grok multi-channel pool now survives access-token refresh boundaries.**
  Grok refresh tokens are single-use with reuse detection; two concurrent
  isolated turns crossing a token expiry could both spend the same refresh
  token, which xAI treats as theft and answers by revoking the whole session
  family — deauthorizing the operator behind a green badge. Admission to the
  Grok pool is now freshness-gated: while the shared access token has
  comfortable lifetime (5-minute margin, `EMPIR3_GROK_REFRESH_MARGIN_MS`),
  turns run concurrently as before; once expiry is inside the margin — or an
  auth-class failure is observed — the pool truthfully serializes to one
  channel so exactly one CLI performs the rotation. An eager watcher persists
  the rotated token to the real auth store the moment it appears in any
  isolated turn home (not just at turn cleanup), reopening the pool within
  seconds; newly admitted turns copy the new token. Turns arriving during the
  refresh window park briefly (`EMPIR3_GROK_STALE_PARK_MS`, default 15s)
  before reporting the typed retryable busy result, and the capability probe
  advertises the serialized capacity plus a `pool_state` field so compatible
  app servers never over-fan a draining pool. `cli_run` also copies its auth
  snapshot after capacity admission rather than before, so a parked run can
  never start with a pre-rotation token.
- Auth write-backs from isolated turn homes now validate as JSON before
  reaching the real credential store, so a torn mid-write read can never
  corrupt it, and repeated in-turn rotations advance the comparison baseline
  instead of being dropped.
- **Malformed handler results are now loud, distinct bug signatures.** The
  0.3.84 Higgsfield outage was masked for hours because a returned-function
  envelope collapsed into a generic `generate_failed`. A non-object result, a
  missing `success` boolean, or a bare failure with no stage/error now
  produces stage `internal_bad_result` with the full envelope logged
  server-side, clearly labeled as a Bridge bug rather than a provider
  failure.
- **The empir3-channel Higgsfield path now enforces the tray handler-family
  gate** (previously only the local `/api/command` path did): a disabled
  handler answers `higgsfield:cli:error` with stage `handler_disabled` and
  the exact tray-menu message, and the capability probe reports
  `handler_enabled` plus `available: false` so route eligibility and the
  console badge show the disabled state instead of green.

### Added

- **Retention sweep for lent-CLI transcripts at rest.** Every lent Claude
  turn writes a full transcript under `~/.claude/projects/<cwd-slug>/`,
  accumulating indefinitely across all users routed through one Bridge. The
  daemon now sweeps transcript files older than the retention window
  (`lentTranscriptRetentionDays` setting, default 7 days, `0` disables) from
  Bridge-owned lent-turn directories only — the payload root (all versions)
  and the configured Empir3 home directory — plus the Bridge's own `cli-runs`
  transcript files. The owner's personal project transcripts are never
  touched. Grok needs no sweep: its per-turn homes are ephemeral by design.

- `scripts/stress-grok-refresh-boundary.mjs` — a forced refresh-boundary
  stress that drives the Bridge's gated `cli_run` path with N concurrent Grok
  turns, optionally rewriting the auth store's `expires_at` metadata to force
  an imminent boundary, and reports completion/rejection/rotation truthfully
  without printing token material.

## [0.3.85] - 2026-08-15

Version 0.3.84 was a source-only step and was never published; its content
shipped with 0.3.85.

### Added

- **Linux X11 desktop control for agent boxes.** Headless Linux Bridges now
  discover a boot-time X11 display (including Xvfb services that do not inherit
  `DISPLAY`) and provide the safe see/point/click/type desktop subset through
  `scrot` and `xdotool`. Unsupported window, clipboard, notification,
  application-control, SOM, and page-to-screen operations still fail closed.
- **Unattended pairing for the Linux headless package.** The headless runtime
  now ships the same `bundle-pair-claim.js` the Windows payload uses, and
  `install.sh --pair <code>` (or `EMPIR3_PAIR_CODE` from provisioning /
  cloud-init) redeems a pre-authorized Empir3 pairing session as the service
  user before the first service start — a fresh box comes up already paired
  with no browser step. `headless-entry.js` gained `--pair <code>` (pair, then
  start) and `--pair-only` (installer one-shot); an environment-supplied code
  is ignored on an already-paired box so service restarts never re-pair. A
...
- **Grok re-auth treadmill ended.** Each lent Grok turn runs in an isolated
  turn home with a copy of the CLI's auth state; Grok rotates refresh tokens
  on use, and the rotated token previously died with the turn home — bricking
  the session and demanding re-authentication despite a valid login. Turn
  cleanup now writes a changed auth state back to the real home via
  compare-and-swap, serialized across concurrent turn cleanups.
- **Higgsfield generation restored on the CLI path.** 0.3.84 shipped
  `higgsfieldGenerate` returning its async body uninvoked, so every CLI-mode
  Higgsfield image/video generation failed instantly and fell over to API
  failover. The generate path now executes and serves normally.
- Tray update handoffs always record the newly running payload version, even
  when the explicit post-update console-reopen flag is set. This prevents a
  stale history cache from reopening the Bridge console again on a later cold
  start; update selection itself continues to use the signed active-payload
  pointer.
...
- Automatic Higgsfield install and update are paused on Windows after
  Microsoft Defender classified the exact `hf.exe` from Higgsfield's official
  v1.1.23 Windows release as `Trojan:Win32/Bearfoos.A!ml`. The console tells
  owners not to bypass Defender while macOS/Linux retain their vendor update
  paths. This does not claim whether the upstream binary is malicious or a
  false positive; Windows execution remains fail-closed until the vendors
  resolve it.

### Fixed

- Visible Windows installers now execute each fixed catalog command as a
  resolved executable plus quoted arguments. The prior nested `cmd /c` shape
  could open the terminal successfully but fail with "The system cannot find
  the path specified" before npm or winget actually ran.
- Batch-backed CLI actions now use `call`, so control returns to the Bridge
  launcher and the real exit result remains visible instead of the terminal
  disappearing at the end of an npm shim.

## [0.3.79] - 2026-08-11

### Fixed

- The API & CLIs pane now expands to the available desktop width instead of
  leaving a large unused column and squeezing lifecycle controls into narrow,
  vertically stacked buttons. Provider rows use stable column proportions,
  compact contextual action labels, and horizontal overflow only when the
...
- The successor tray detects a payload version transition itself and restores
  the visible Bridge console, including upgrades from older trays that cannot
  pass the new relaunch handoff flag.
- Atomic settings writes now retry short-lived Windows destination locks while
  preserving the existing primary file. Antivirus or indexing reads no longer
  turn a settings acknowledgement into a rapid EPERM/retry loop.

### Changed

- Production Bridge publication now requires Empir3 maintenance mode and a
  zero in-flight turn drain before the signed manifest is exposed. Corrected
  payloads must always receive a new version instead of replacing cached bytes.
- The Windows bootstrap is now 2.0.3 so launching the signed installer replaces
  older 2.0.2 stable copies instead of treating an unsigned legacy copy as an
  equal/current bootstrap.

## [0.3.77] - 2026-08-11

### Added

- The API & CLIs console can explicitly check vendor release sources, show
  installed versus latest versions, and launch fixed per-provider update and
  sign-out actions. Gemini CLI is now visible alongside Antigravity instead of
  being silently probed off-screen.
- Update checks identify their source and distinguish current, update
  available, unavailable, and vendor-managed states. Antigravity is reported
  honestly as vendor-managed because its CLI has no read-only latest command.

### Fixed

- Windows authentication, update, and sign-out actions now open in a real,
  persistent console created by Start-Process. Higgsfield and other TTY/browser
  flows no longer flash or inherit NUL standard handles from the tray daemon.
- A payload update now detects the version transition inside the successor,
  then restores and raises the visible Bridge console after its daemon is
  healthy instead of completing invisibly in the background. This also covers
  the first upgrade from an older tray that cannot pass the new handoff flag.
  Windows Store `winget` aliases are accepted for the fixed GitHub CLI update
  action.
- A partial Higgsfield npm install with a missing native binary is now shown as
  incomplete/not installed so the owner is offered a reinstall instead of a
...
- Browser text, accessibility snapshots, and screenshots now give the CDP
  operation enough time to answer instead of aborting first and immediately
  stacking duplicate reads behind the still-running command. A timed-out
  screenshot also fails once without launching a second full capture, keeping
  Gmail and other dense pages from wedging the serialized browser queue.

## [0.3.75] - 2026-08-11

### Fixed

- Claude Code MCP tool turns now align the CLI's idle watchdog with the
  Bridge's existing 20-minute tool ceiling. Long-running specialist dispatches
  no longer fail after five minutes merely because the MCP stream is idle.

## [0.3.74] - 2026-08-10

### Security

- CLI child processes are now tracked as exact owned process trees and are
  terminated by PID without a command shell. Timeouts and cancellations no
  longer risk killing unrelated Node, Grok, Antigravity, GitHub, Higgsfield,
  or SSH work on the machine.
- Concurrent Grok turns now use isolated temporary home/config roots and
  unique leader sockets. Only authentication material is copied in, and stale
  Empir3 MCP entries are removed atomically at Bridge startup.
- Packaged smoke tests now fail closed unless every writable path is contained
  by their scratch root and refuse to run over an existing Bridge instance.

### Changed

- CLI model discovery now returns sanitized catalogs where the CLI exposes a
  real catalog, and an explicit `catalog_not_exposed_by_cli` result where it
  does not, instead of presenting guessed model lists.
- Desktop update enqueue requests now require a recent tray poll so a stale
  or disconnected tray receives an honest conflict instead of a false
  acceptance.
- Retired the legacy project bootstrap mirror while preserving the current
  WebSocket workspace and sync contract.

### Fixed

- Added a bounded, credential- and home-path-sanitized Bridge diagnostics tail
  with aggregate CLI process telemetry for the owner-scoped admin route.
- AGY image generation, Higgsfield, GitHub CLI, VPS SSH, generic CLI, and Grok
  execution now share the exact process ownership and cleanup contract.

## [0.3.73] - 2026-08-09

### Fixed

- Higgsfield prompts on Windows now bypass the npm command shell, so prompt
  punctuation cannot be misread as commands such as `HOME` or `HAND`.
- Antigravity image harvesting now detects images written into reused brain
  conversations, waits briefly for post-exit file flushing, and reports model
  quota/permission failures honestly so Empir3 can fail over promptly.

## [0.3.72] - 2026-08-08

### Security

- Retired the credentialed page-world chat overlay. Arbitrary websites no
  longer receive a Bridge nonce, message queue, agent replies, or a
  command-capable WebSocket. Chat now runs only on the trusted localhost
  dashboard; the compatibility reinject command removes legacy overlay state
  from already-open tabs and clears CDP auto-injection.
- Browser WebSocket roles now fail closed: native CLI clients must have no
  browser `Origin`, the control role requires the exact localhost dashboard
  origin, and the former overlay role is rejected.
- Dashboard chat messages are built with text-only DOM nodes under a
  nonce-scoped Content Security Policy, preventing forwarded chat HTML from
  executing against privileged localhost APIs.
- Browser navigation now requires Write permission, accepts only HTTP(S),
  blocks embedded credentials and literal/resolved loopback, private-network,
  link-local, and metadata addresses, while retaining access to the Bridge's
  own localhost control page.
- Conversation IDs and recording/feedback filenames are now flat,
  length-bounded resources that cannot escape their storage directories.
- Updated `fast-uri` and `hono`; the production dependency audit is clean.

### Changed

- Browser chat moved from the controlled webpage into
  `http://localhost:3006/`. The desktop toolbar and setup console now open that
  trusted surface instead of injecting controls into page content.
- The maintained npm distribution moved to
  `@empir3hq/empir3-bridge`, matching the public GitHub organization. The old
  `@empir3/empir3-bridge` package remains a historical 0.3.22 release and is no
  longer the supported npm install path.

### Tests

- Added hostile-origin WebSocket, retired-overlay, HTML-injection,
  private-network navigation, and encoded path-traversal regression coverage,
  including an isolated live wrapper-server test.

## [0.3.71] - 2026-08-07

### Fixed

- OpenAI-compatible image providers now use the multipart image-edit endpoint
  when Empir3 sends reference pixels, preserving the original image instead of
  rejecting the edit or falling back to a new generation.
- Reference images are decoded through the Bridge's existing inbound size
  limits, optional edit controls are forwarded safely as multipart fields, and
  generation-only requests retain the existing JSON generation route.
