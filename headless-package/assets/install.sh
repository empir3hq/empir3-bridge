...
install -d -m 0700 -o "$SERVICE_USER" -g "$SERVICE_USER" "$SERVICE_HOME/.empir3-bridge"
...
cat > /etc/systemd/system/empir3-bridge-update.path <<EOF
[Unit]
Description=Empir3 Bridge on-demand signed update request

[Path]
PathChanged=$SERVICE_HOME/.empir3-bridge/update-request
Unit=empir3-bridge-update.service
...
rm -rf -- "$BACKUP"
systemctl enable --now empir3-bridge-update.timer empir3-bridge-update.path
