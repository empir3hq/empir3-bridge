...
systemctl disable --now empir3-bridge-update.timer 2>/dev/null || true
systemctl disable --now "$UNIT" 2>/dev/null || true
rm -f -- "/etc/systemd/system/$UNIT" /etc/systemd/system/empir3-bridge-update.service /etc/systemd/system/empir3-bridge-update.timer /etc/empir3-bridge.env /etc/empir3-bridge-install.json
