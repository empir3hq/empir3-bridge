 ...
 *   4.  a desktop_* tool returns structured capability_unsupported on
 *       non-Windows — never a spawn ENOENT (Windows: expects it to pass gate)
...
async function main() {
  ...
  try {
    ...
    check('bridge boots to /api/status', !!status && !exited, exited || output.slice(-500));
    ...
    if (POSIX) {
      check('desktop_screenshot → capability_unsupported', shot?.code === 'capability_unsupported' && shot?.success === false, shot);
      check('refusal carries deviceClass + hint', typeof shot?.deviceClass === 'string' && typeof shot?.hint === 'string', shot);
