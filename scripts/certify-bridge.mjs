...
async function main() {
  ...
  writeJson('installer-version.json', {
    ...
    // Hash in-process. Launching Windows PowerShell from a PowerShell 7 parent
    // can inherit a PSModulePath that omits Microsoft.PowerShell.Utility,
    // making the standard Get-FileHash cmdlet disappear on an otherwise healthy
    // machine. Node's crypto path is cross-shell and verifies the same bytes.
    sha256: createHash('sha256').update(readFileSync(installer)).digest('hex'),
  });
