...
if (desktopOnly) {
  ...
  uploadAndVerify('Stage 1: desktop artifacts, then the versioned artifact index', [...desktopFiles, artifactIndexFile]);
  for (const artifact of desktopFiles) publicShaCheck(artifact.url, artifact.sha);
...
uploadAndVerify('Stage 1: every referenced artifact, then the versioned artifact index',
  [nodeTarFile, nodeSigFile, payloadTarFile, payloadSigFile, ...desktopFiles, ...(artifactIndexFile ? [artifactIndexFile] : [])]);
