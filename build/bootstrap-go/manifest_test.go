...
func TestVerifyRoundTripWithSchema3DesktopFields(t *testing.T) {
	pub, priv := testKey(t)
	fields := fullFixture()
	fields["schemaVersion"] = "3"
	fields["releaseChannel"] = "test"
	fields["rolloutState"] = "hold"
	fields["rolloutPercent"] = "0"
	fields["desktopDarwinUniversalInstallerUrl"] = "https://app.empir3.com/downloads/empir3-bridge-macos-universal.dmg"
	fields["desktopLinuxArm64Health"] = "package-smoke-passed"
	raw := signFixture(t, priv, fields)
	if _, err := ParseAndVerifyManifest(raw, pub); err != nil {
		t.Fatalf("legacy Go verifier rejected backward-compatible schema 3 fields: %v", err)
	}
}
