...
const { buildReleaseManifestV3, canPromoteDesktopManifestToLegacy } = require('./release-manifest-v3');
...
const DESKTOP_VERSION_MANIFEST = path.join(DIST_DIR, 'bridge-desktop-version.json');
...
// Authenticode-sign the exe via Azure Trusted Signing. The signing config
// (build/signing/) is maintainer-only and not part of the public tree; auth
// is DefaultAzureCredential (`az login` as an identity holding the
// "Trusted Signing Certificate Profile Signer" role). Dev builds without the
...
function buildManifest(payload, node, privateKey, pubKeyHex) {
  ...
  const artifactIndexInput = process.env.EMPIR3_DESKTOP_ARTIFACT_INDEX;
  if (artifactIndexInput) {
    const inputPath = path.resolve(BRIDGE_DIR, artifactIndexInput);
    const indexBytes = fs.readFileSync(inputPath);
    const index = JSON.parse(indexBytes.toString('utf8'));
    const indexName = `empir3-bridge-artifacts-v${payload.version}.json`;
    const indexOutput = path.join(DIST_DIR, indexName);
    fs.writeFileSync(indexOutput, indexBytes);
    fields = buildReleaseManifestV3(fields, index, {
      artifactIndexUrl: `${base}/${indexName}${bust(payload.version)}`,
      artifactIndexSha256: sha256OfBuf(indexBytes),
    });
    step(`Schema 3 desktop artifact index embedded → ${path.relative(BRIDGE_DIR, indexOutput)}`);
  }
  ...
  const manifestBytes = JSON.stringify(fields, null, 2) + '\n';
  const isDesktopSchema = fields.schemaVersion === '3';
  const manifestPath = isDesktopSchema ? DESKTOP_VERSION_MANIFEST : VERSION_MANIFEST;
  fs.writeFileSync(manifestPath, manifestBytes);
  step(`Manifest written → ${path.relative(BRIDGE_DIR, manifestPath)}`);

  if (isDesktopSchema) {
    const fullyLive = canPromoteDesktopManifestToLegacy(fields);
    if (fullyLive) {
      fs.writeFileSync(VERSION_MANIFEST, manifestBytes);
      step('Fully-live schema 3 also promoted to legacy bridge-version.json');
    } else {
      // A stale file from a prior build would be dangerous: the publisher must
      // fail closed instead of accidentally exposing a held/staged payload to
      // legacy Windows bootstrappers that ignore rollout fields.
      rmIfExists(VERSION_MANIFEST);
      step(`Legacy bridge-version.json withheld for ${fields.rolloutState}/${fields.rolloutPercent}% desktop rollout`);
    }
  } else {
    rmIfExists(DESKTOP_VERSION_MANIFEST);
  }
  return { fields, manifestPath };
}

// ── Self-verify everything the stub will check, the way the stub checks it ──

function selfVerifyAll(payload, node, pubKeyHex, manifestPath = VERSION_MANIFEST) {
...
async function main() {
  ...
  const releaseManifest = buildManifest(payload, node, privateKey, pubKeyHex);
  const manifest = releaseManifest.fields;

  selfVerifyAll(payload, node, pubKeyHex, releaseManifest.manifestPath);
