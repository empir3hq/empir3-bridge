...
TRAY_VERSION_STATE_FILE = APPDATA / 'tray-version.txt'
...
def _record_tray_version_and_detect_transition(current: str) -> bool:
    """Persist the running tray version and detect an update/rollback start.

    The predecessor tray may predate the environment handoff flag. On the first
    upgrade into this implementation, the presence of an older cached payload
    is the durable evidence that this is a version transition rather than a
    clean first install.
    """
    if not current or current in ('dev', 'unknown'):
        return False
    previous = ''
    try:
        if TRAY_VERSION_STATE_FILE.exists():
            previous = TRAY_VERSION_STATE_FILE.read_text(encoding='utf-8').strip()
    except Exception as e:
        logger.warning('tray version state read failed: %s', e)

    transitioned = bool(previous and previous != current)
    if not previous:
        try:
            current_tuple = _ver_tuple(current)
            transitioned = any(
                child.is_dir()
                and child.name != current
                and _ver_tuple(child.name) < current_tuple
    ...
    try:
        temp = TRAY_VERSION_STATE_FILE.with_suffix(f'.tmp-{os.getpid()}')
        temp.write_text(current, encoding='utf-8')
        temp.replace(TRAY_VERSION_STATE_FILE)
    except Exception as e:
        logger.warning('tray version state write failed: %s', e)
    if transitioned:
        logger.info('tray version transition detected: %s -> %s', previous or 'cached payload', current)
    return transitioned
...
def _acquire_single_instance(handoff_wait_sec: float = 15.0) -> bool:
    """Return False when another tray is already supervising the bridge.

    Update-handoff race (observed on the vault VPS, 0.3.43 and 0.3.45): the
    old tray spawns the new tray and exits, but it holds this mutex until its
    process actually dies — pystray shutdown + interpreter teardown can take
    seconds on a slow VM, while the new frozen exe reaches this check in ~1s.
    An immediate exit-on-ERROR_ALREADY_EXISTS therefore killed the NEW tray,
    then the old one finished dying, and no tray (or daemon) was left at all.

    Fix: when the mutex already exists, WAIT on it instead of exiting. If the
    holder is a dying tray, ownership transfers to us the moment it exits
    (normal release via atexit, or WAIT_ABANDONED if it was hard-killed). Only
    a holder that is still alive after `handoff_wait_sec` is a genuine second
    instance — then we exit as before.
    """
    ...
    try:
        ...
        if already_exists:
            logger.info(
                'single-instance mutex is held (update handoff?) — waiting up to %.0fs for the previous tray to release it',
                handoff_wait_sec,
            )
            WAIT_OBJECT_0 = 0x00000000
            WAIT_ABANDONED = 0x00000080
            rc = kernel32.WaitForSingleObject(handle, int(handoff_wait_sec * 1000))
            if rc in (WAIT_OBJECT_0, WAIT_ABANDONED):
                logger.info(
                    'single-instance mutex acquired after handoff (%s)',
                    'released' if rc == WAIT_OBJECT_0 else 'abandoned by dead holder',
                )
            else:
...
def _release_instance_mutex() -> None:
    """Release + close the single-instance mutex. Idempotent — called both
    from atexit and explicitly by the update handoff right before the new
    tray is spawned, so the successor never has to wait on our teardown."""
    global _INSTANCE_MUTEX_HANDLE
...
def _create_icon_image(connected: bool) -> Image.Image:
    """Load the same high-contrast Bridge glyph as the Electron shell."""
    filename = 'bridge-tray-connected.png' if connected else 'bridge-tray-disconnected.png'
    if getattr(sys, 'frozen', False):
        icon_path = Path(getattr(sys, '_MEIPASS', Path(sys.executable).parent)) / 'bridge-icons' / filename
    else:
        icon_path = Path(__file__).resolve().parent.parent / 'assets' / 'icons' / filename
...
class EmpirTray:
        ...
        def _do():
            ...
            try:
                req = urlrequest.Request(
                    f'http://127.0.0.1:{port}/api/browser/show',
                    method='POST',
                    data=b'{}',
                    headers={
        ...
        while time.time() < deadline:
            try:
                ...
                if is_newer(active, self._tray_version) and new_tray.exists():
                    ...
                    if self._icon:
                        self._restart_tray(self._icon, reopen_welcome=True)
                    return
            except Exception as e:
    ...
    def _restart_tray(self, icon, _item=None, *, reopen_welcome: bool = False):
        ...
        def _do():
            ...
            # Hand the single-instance mutex over BEFORE spawning the successor.
            # Our poller + supervisor are already stopped, so nothing we still
            # do needs the lock — and holding it through pystray/interpreter
            # teardown is exactly the race that made the new tray exit with
            # "another instance is already running" on slow VMs (vault VPS,
            # every update). The new tray's wait-on-mutex is the backstop;
            # this makes the normal path instant.
            ...
            try:
                ...
                if reopen_welcome:
                    # The daemon/browser are torn down during a payload handoff.
                    # Tell the successor to restore the console the owner was
                    # using instead of completing the update invisibly.
                    tray_env['EMPIR3_REOPEN_WELCOME_AFTER_UPDATE'] = '1'
                subprocess.Popen(
                    [str(new_tray)],
    ...
    def _restore_welcome_after_update(self):
        """Wait for the successor daemon, then navigate and raise its console."""
        deadline = time.time() + 45
        while time.time() < deadline:
            port = self._last_status.get('port')
            if port:
                if self._open_controlled_url(port, self._welcome_url(port), 'update-relaunch'):
                    logger.info('update-relaunch: restored visible bridge console')
                    return
            time.sleep(0.5)
        logger.warning('update-relaunch: daemon did not become ready within 45s')
    ...
    def run(self):
        ...
        if not _acquire_single_instance():
            return
        explicit_reopen = os.environ.pop('EMPIR3_REOPEN_WELCOME_AFTER_UPDATE', '') == '1'
        # Always persist the running version. Using ``or`` directly here used
        # to short-circuit the recorder on normal update handoffs, leaving the
        # cache one release behind even though the new tray was running.
        recorded_transition = _record_tray_version_and_detect_transition(self._tray_version)
        reopen_after_update = explicit_reopen or recorded_transition
        ...
        if reopen_after_update:
            threading.Thread(target=self._restore_welcome_after_update, daemon=True).start()
        try:
            self._icon.run()
        finally:
