...
test('Grok auth classifier catches the live invalid_grant revocation signature', () => {
  // Exact shape the CLI logged on the revoked local session, 2026-08-16.
  const observed = 'WARN OIDC: token refresh HTTP error http_status=400 oauth2_error=Some("invalid_grant") rt_prefix="XXXXXXXX" client_id=b1a00492';
  assert.equal(classifyGrokAuthFailure(observed), 'refresh_rejected');
  assert.equal(classifyGrokAuthFailure('token refresh failed after 3 attempts'), 'refresh_failed');
  // A refresh error mentioning a rate limit stays unclassified (billing guard).
  assert.equal(classifyGrokAuthFailure('token refresh error: rate limit exceeded'), null);
});
