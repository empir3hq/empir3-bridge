...
test('Windows listener parsing returns only exact requested LISTENING ports', () => {
  const raw = [
    '  Proto  Local Address          Foreign Address        State           PID',
    '  TCP    127.0.0.1:3006         0.0.0.0:0              LISTENING       4100',
    '  TCP    0.0.0.0:9867           0.0.0.0:0              LISTENING       4200',
    '  TCP    [::]:3006              [::]:0                 LISTENING       4100',
    '  TCP    127.0.0.1:3316         0.0.0.0:0              LISTENING       4300',
    '  TCP    127.0.0.1:10167        127.0.0.1:50123        ESTABLISHED     4400',
    '  UDP    0.0.0.0:10167          *:*                                    4500',
  ].join('\r\n');

  assert.deepEqual(
    [...parseWindowsListeningPids(raw, [3006, 9867])].sort((a, b) => a - b),
    [4100, 4200],
  );
});

test('Windows parallel launch reaps only a Bridge process proven on its requested ports', () => {
  const root = 'D:\\Github\\empir3-bridge';
  const processes = [
    { pid: 5100, cmd: 'node C:\\Users\\x\\.empir3-bridge\\payload\\0.3.97\\entry.js --daemon' },
    { pid: 5200, cmd: 'node C:\\Users\\x\\.empir3-bridge\\node\\mcp-server.js' },
    { pid: 5300, cmd: `node ${root}\\src\\server.ts` },
  ];
  ...
  const owned = listOwnedProcesses({
    ...
    environmentForPid: () => ({}),
    windowsPortOwners: new Set([5300]),
  });

  assert.deepEqual(owned.map(process => process.pid), [5300]);
});

test('Windows process cleanup fails closed when listener ownership cannot be read', () => {
  const owned = listOwnedProcesses({
    pwPort: 3316,
    bridgePort: 10167,
    platform: 'win32',
    processRows: [
      { pid: 6100, cmd: 'node C:\\Users\\x\\.empir3-bridge\\payload\\0.3.97\\entry.js --daemon' },
      { pid: 6200, cmd: 'node C:\\Users\\x\\.empir3-bridge\\node\\mcp-server.js' },
    ],
    environmentForPid: () => ({}),
    windowsPortOwners: null,
  });
