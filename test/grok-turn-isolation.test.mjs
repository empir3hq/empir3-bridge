...
test('five concurrent Grok turns receive separate homes, configs, auth copies, and leader sockets', async () => {
  ...
  try {
    ...
    turns.forEach((turn, index) => {
      ...
      assert.equal(turn.env.GROK_HOME, join(turn.root, '.grok'));
      assert.equal(turn.env.CLAUDE_CONFIG_DIR, join(turn.root, '.claude'));
      assert.equal(turn.env.GROK_CLAUDE_MCPS_ENABLED, 'false');
      assert.equal(turn.env.GROK_CURSOR_MCPS_ENABLED, 'false');
      assert.deepEqual(turn.extraArgs, ['--always-approve', '--tools', '', '--leader-socket', turn.leaderSocket]);
      assert.match(configs[index], new RegExp(String(41001 + index)));
      assert.match(configs[index], /\[compat\.claude\][\s\S]*?mcps = false/);
      assert.match(configs[index], /\[compat\.cursor\][\s\S]*?mcps = false/);
...
test('only explicitly approved agentic Grok isolation retains native tools', async () => {
  const root = await mkdtemp(join(tmpdir(), 'empir3-grok-agentic-isolation-test-'));
  const realHome = join(root, 'real-home');
  const turnRoot = join(root, 'turns');
  await mkdir(join(realHome, '.grok'), { recursive: true });
  await mkdir(turnRoot, { recursive: true });
  await writeFile(join(realHome, '.grok', 'auth.json'), '{}', 'utf-8');
  const turn = await createGrokTurnIsolation('approved-agentic', '', {
    realHome,
    tempRoot: turnRoot,
  ...
  try {
    assert.deepEqual(turn.extraArgs, ['--always-approve', '--leader-socket', turn.leaderSocket]);
    const config = await readFile(turn.configPath, 'utf-8');
