...
test('the installer lets runtime display discovery classify agent boxes', () => {
  assert.doesNotMatch(installer, /^EMPIR3_HEADLESS=/m);
  assert.doesNotMatch(installer, /^BRIDGE_HEADLESS=/m);
  assert.doesNotMatch(installer, /^EMPIR3_CHROME_AUTOLAUNCH=/m);
  assert.match(installer, /^BindReadOnlyPaths=-\/tmp\/\.X11-unix$/m);
});
...
test('headless install has bounded health, restart detection, and signed update timer contracts', () => {
  ...
  assert.match(installer, /^PathChanged=\$SERVICE_HOME\/\.empir3-bridge\/update-request$/m);
  assert.match(installer, /systemctl enable --now empir3-bridge-update\.timer empir3-bridge-update\.path/);
});

test('the console routes managed Linux updates through the systemd request path', () => {
  assert.match(server, /url\.pathname === '\/api\/updates\/apply'/);
  assert.match(server, /process\.platform === 'linux' && existsSync\(HEADLESS_INSTALL_CONFIG\)/);
  assert.match(server, /writeFileSync\(HEADLESS_UPDATE_REQUEST_FILE/);
  assert.match(server, /method: 'systemd-path'/);
  assert.match(server, /postJson\('\/api\/updates\/apply', \{\}\)/);
});
