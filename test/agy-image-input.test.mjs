...
import { readFileSync, mkdtempSync, writeFileSync, rmSync, utimesSync, readdirSync } from 'node:fs';
...
test('both harvest sites exclude the planted reference', () => {
  const harvestCalls = handler.match(/findHarvestImage\(workDir, preBrainConvs, referencePath\)/g) || [];
...
test('findHarvestImage never returns the excluded reference, even with no other candidate', async () => {
  const { findHarvestImage } = await import('../src/handlers/agy-imagegen.ts')
    .catch(() => ({ findHarvestImage: null }));
  if (!findHarvestImage) return; // no TS loader in this runner — source contracts above still hold
  ...
  let preBrain = new Set();
  try { preBrain = new Set(readdirSync(join(homedir(), '.gemini', 'antigravity-cli', 'brain'))); } catch { /* absent */ }
  try {
