...
test('Codex text, tool, and vision turns ignore owner config and use a private cwd', () => {
  ...
  assert.match(turn, /buildCodexBridgePrompt\(String\(payload\?\.system \|\| ''\), messages, turnTools\.length > 0\)/);
...
test('Codex project-work prompt makes attached tool execution mandatory', () => {
  ...
  assert.match(prompt, /CRITICAL EMPIR3 TOOL CONTRACT:/);
  assert.match(prompt, /A prose-only response is rejected by the host/);
  assert.match(prompt, /hasTools \? \[/);
});
...
test('Gemini, Grok, and AGY project-work prompts require attached MCP tools', () => {
  ...
  assert.match(prompt, /CRITICAL EMPIR3 TOOL CONTRACT:/);
  assert.match(prompt, /For any requested project work, you MUST call the appropriate Empir3 project tools/);
  assert.match(prompt, /A prose-only response is rejected by the host/);
  assert.match(prompt, /toolNames\.length > 0 \? \[/);
});
