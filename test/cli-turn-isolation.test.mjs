...
test('Codex text, tool, and vision turns ignore owner config and use a private cwd', () => {
  ...
  assert.match(turn, /buildCodexBridgePrompt\(String\(payload\?\.system \|\| ''\), messages, turnTools\)/);
...
test('Codex project-work prompt makes attached tool execution mandatory', () => {
  // The contract wording lives in the shared role-aware helper; the builder
  // spreads it in ahead of the system prompt whenever tools are attached.
  const contract = server.match(/function empir3ToolContractLines[\s\S]*?function cliToolNames/)?.[0] || '';
  assert.match(contract, /CRITICAL EMPIR3 TOOL CONTRACT:/);
  assert.match(contract, /A prose-only response is rejected by the host/);
  assert.match(contract, /if \(toolNames\.length === 0\) return \[\]/);
  const prompt = server.match(/function buildCodexBridgePrompt[\s\S]*?function cliTomlStringValue/)?.[0] || '';
  assert.match(prompt, /\.\.\.empir3ToolContractLines\(cliToolNames\(tools\)\)/);
});

test('per-turn MCP shim instructs attached CLIs to execute required project work', () => {
  const shim = server.match(/async function startCliMcpShim[\s\S]*?async function startClaudeCliMcpShim/)?.[0] || '';
  assert.match(shim, /instructions: 'Empir3 provides the authorized tools for this turn/);
  assert.match(shim, /A prose-only response does not satisfy a project-work request/);
});

test('Gemini, Grok, and AGY project-work prompts require attached MCP tools', () => {
  const contract = server.match(/function empir3ToolContractLines[\s\S]*?function cliToolNames/)?.[0] || '';
  assert.match(contract, /For any requested project work, you MUST call the appropriate Empir3 project tools/);
  assert.match(contract, /A prose-only response is rejected by the host/);
  const prompt = server.match(/function buildPlainCliPrompt[\s\S]*?async function startPlainCliTurn/)?.[0] || '';
  assert.match(prompt, /\.\.\.empir3ToolContractLines\(toolNames\)/);
  assert.match(prompt, /Empir3 MCP tools are available and required for real project work/);
});
...
test('AGY preserves auth receipts but isolates home config, MCPs, plugins, and skills', () => {
  ...
  assert.match(setup, /copyTurnAuthFile\(tempDir, \['\.gemini', 'google_accounts\.json'\]\)/);
  assert.match(setup, /copyTurnAuthFile\(tempDir, \['\.gemini', 'installation_id'\]\)/);
  assert.match(setup, /join\(tempDir, '\.gemini', 'config'\)/);
  assert.match(setup, /join\(agyConfigDir, 'mcp_config\.json'\)/);
  assert.doesNotMatch(setup, /join\(tempDir, '\.antigravity'\)/);
