...
test('Windows persistent JSON retries a transient destination file lock', { skip: process.platform !== 'win32' }, () => {
  const dir = scratchDir();
  const primary = join(dir, 'bridge-settings.json');
  const fs = require('node:fs');
  const modulePath = require.resolve('../src/persistent-json.js');
  const originalRename = fs.renameSync;
  let injectedLocks = 0;
  try {
    fs.renameSync = (source, destination) => {
      if (destination === primary && injectedLocks < 3) {
        injectedLocks += 1;
        const error = new Error('destination is briefly locked');
        error.code = 'EPERM';
        throw error;
      }
    ...
    const retryingPersistentJson = require('../src/persistent-json.js');
    retryingPersistentJson.writePersistentJson(primary, providerState(), { mode: 0o600 });

    assert.equal(injectedLocks, 3);
    assert.equal(JSON.parse(readFileSync(primary, 'utf8')).deviceId, 'bridge-retention-test');
