...
test('Grok authorization UI exposes explicit live verification truth', () => {
  ...
  assert.match(server, /if \(button && button\.disabled\) return/);
  assert.match(server, /button\.textContent = 'Verifying…'/);
});

test('Grok authorization verification is single-flight and tool-isolated', () => {
  assert.match(server, /let grokAuthVerifyInFlight: Promise<GrokAuthVerificationResult> \| null = null/);
  assert.match(server, /if \(grokAuthVerifyInFlight\) return grokAuthVerifyInFlight/);
  assert.match(server, /const run = verifyGrokAuthLiveOnce\(\)/);
  // createGrokTurnIsolation supplies --tools "" in the same candidate; the
  // verifier must keep consuming its complete argv rather than reconstructing
  // a weaker subset.
  assert.match(server, /'--disable-web-search',[\s\S]{0,120}\.\.\.isolation\.extraArgs/);
});
