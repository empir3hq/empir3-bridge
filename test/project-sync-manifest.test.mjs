...
test('project manifest ignores excluded paths and oversized files', async () => {
  ...
  try {
    ...
    assert.deepEqual(manifest['project-2'].files.map((file) => file.path), ['keep.txt']);
    assert.deepEqual(skipped, ['large.bin']);
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
