...
test('modifier shortcuts carry virtual-key codes and failed verified typing restores the original value', () => {
  assert.match(bridge, /const virtualKeyCode = named\?\.keyCode \|\| \(mainKey\.length === 1 \? mainKey\.toUpperCase\(\)\.charCodeAt\(0\) : 0\)/);
  assert.equal((bridge.match(/nativeVirtualKeyCode: virtualKeyCode/g) || []).length, 2);
  assert.equal((bridge.match(/windowsVirtualKeyCode: virtualKeyCode/g) || []).length, 2);
  assert.match(bridge, /Object\.getOwnPropertyDescriptor\(proto, 'value'\)\?\.set/);
  assert.match(bridge, /setter\.call\(el, originalValue\)/);
  assert.match(bridge, /new Event\('input', \{ bubbles: true \}\)/);
  assert.match(bridge, /catch \{ \/\* preserve the original verified failure \*\/ \}/);
});
