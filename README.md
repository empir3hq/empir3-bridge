...
https://github.com/user-attachments/assets/1db136ff-c704-4232-9444-1453a3ea2fa7

<sub>**▶ 60 seconds** — browser use, computer use, and multi-model orchestration: your subscriptions, your machine, no paywall.</sub>
...
> **Packaged test builds now exist for Windows, macOS, and desktop Linux, plus a lightweight Linux server/VPS package.** They all host the same Bridge engine and provider console. The Windows candidate passed Authenticode signing and a real install/uninstall/reinstall cycle. The universal macOS candidate passed Developer ID signing, Apple notarization, stapling, install/reinstall, update, and rollback. Linux packages are authenticated by the signed release manifest. Rollout remains held at 0% and these candidates have not been published, so install from source for normal use today.
