...
test('an occupied default CDP port selects a free fallback without moving public Bridge ports', async () => {
  const checked = [];
  const selected = await selectAvailableCdpPort({
    ports: PRODUCTION_PORTS,
    env: {},
    isAvailable: async (port) => { checked.push(port); return port === 9224; },
  });
  assert.deepEqual(selected, { wrapper: 3006, bridge: 9867, cdp: 9224 });
  assert.deepEqual(checked, [9222, 9223, 9224]);
});

test('an explicit CDP override is never silently changed', async () => {
  const ports = { wrapper: 3006, bridge: 9867, cdp: 10922 };
  let probes = 0;
  assert.equal(await selectAvailableCdpPort({
    ports,
    env: { EMPIR3_CDP_PORT: '10922' },
