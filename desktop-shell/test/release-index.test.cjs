...
test('production live releases use health states understood by deployed updaters', () => {
  assert.throws(() => buildArtifactIndex(fullReceipts(), {
    channel: 'production',
    rolloutState: 'live',
    rolloutPercent: 100,
    health: 'release-acceptance-passed',
  }), /not understood by deployed updaters/);
  const index = buildArtifactIndex(fullReceipts(), {
    channel: 'production',
    rolloutState: 'live',
    rolloutPercent: 100,
    health: 'release-approved',
  });
  assert.equal(index.health['headless-linux-x64'], 'release-approved');
});
