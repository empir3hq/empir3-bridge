...
const { createFakeCodexCli } = require('./smoke-codex-cli-fixture.cjs');
...
async function main() {
  ...
  try {
    const smokeEnvironment = {
      ...
      // Hosted Windows runners do not guarantee an interactive desktop. The
      // scale smoke still launches real Chrome and proves CDP/browser health,
      // but does so headlessly in CI. Real installs never set this switch.
      EMPIR3_SCALE_BROWSER_HEADLESS: process.env.CI === 'true' ? '1' : '',
    ...
    prepareSmokeEnvironmentDirectories({
      ...
      stateRoot: smokeEnvironment.EMPIR3_DESKTOP_SMOKE_STATE_ROOT,
      userData: smokeEnvironment.EMPIR3_DESKTOP_USER_DATA,
      env: smokeEnvironment,
    });
    const result = await runPackagedApp(candidates[0], smokeEnvironment);
