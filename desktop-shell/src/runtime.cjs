...
async function selectAvailableCdpPort({
  ...
  // An explicit operator override is a contract, not a hint. Fail normally if
  // it is occupied so a typo cannot make the Bridge wander to another port.
  if (String(env.EMPIR3_CDP_PORT || '').trim()) return ports;
  if (await isAvailable(ports.cdp)) return ports;
  for (let offset = 1; offset <= searchWidth; offset += 1) {
    const candidate = ports.cdp + offset;
    if (candidate > 65535 || candidate === ports.wrapper || candidate === ports.bridge) continue;
    if (await isAvailable(candidate)) return { ...ports, cdp: candidate };
  }
  throw new Error(`Chrome CDP port ${ports.cdp} is occupied and no fallback port is available`);
}
