...
async function run() {
  ...
  const requestedCdpPort = ports.cdp;
  ports = await selectAvailableCdpPort({ ports });
  if (ports.cdp !== requestedCdpPort) {
    fileLogger.write('info', `Chrome CDP port ${requestedCdpPort} was occupied; selected ${ports.cdp}`);
  }
