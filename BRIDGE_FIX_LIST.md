...
| BR-015 | P0 | The page-injected overlay shared privileged Bridge chat/control state with arbitrary website JavaScript, and browser-origin WebSockets could claim trusted roles. | Remove privileged state from page context, retire existing overlays, require the exact Bridge localhost origin for browser control sockets, keep native CLI sockets non-browser-only, and regression-test hostile origins against a real isolated server. | Fixed in 0.3.72 source; signed release and installed acceptance in progress |
| BR-016 | P1 | Browser navigation and local resource routes accepted inputs without one shared policy covering dangerous schemes, private-network destinations, DNS resolution, and flat local identifiers. | Centralize browser navigation validation, block local/private destinations and unsafe schemes, validate resolved addresses, constrain local resource names, and add adversarial tests. | Fixed in 0.3.72 source; signed release and installed acceptance in progress |
