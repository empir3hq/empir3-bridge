...
**Current release rule:** Bridge 0.3.71 is published and accepted. A candidate
...
| BR-015 | P0 | The page-injected overlay shared privileged Bridge chat/control state with arbitrary website JavaScript, and browser-origin WebSockets could claim trusted roles. | Remove privileged state from page context, retire existing overlays, require the exact Bridge localhost origin for browser control sockets, keep native CLI sockets non-browser-only, and regression-test hostile origins against a real isolated server. | Closed and shipped in 0.3.72; signed installed acceptance passed with pairing retained |
| BR-016 | P1 | Browser navigation and local resource routes accepted inputs without one shared policy covering dangerous schemes, private-network destinations, DNS resolution, and flat local identifiers. | Centralize browser navigation validation, block local/private destinations and unsafe schemes, validate resolved addresses, constrain local resource names, and add adversarial tests. | Closed and shipped in 0.3.72; cross-platform and installed release gates passed |
...
| 2026-08-07 | 0.3.71 exact reference-image edit path | Source and desktop suites passed (248 total Bridge tests, 43/43 desktop tests); installed signed Windows 0.3.71 retained pairing and routed a real production edit to the exact demo-account device and private Spark 2 provider, returning the validated output hash `76a4e9fdb291f9dc763f7ad542ef1c21bd74604750b8a84f23e994178d88cd91` |
| 2026-08-07 | 0.3.71 signed native matrix + production publication | Full CI `31176716953` and signed/notarized macOS lifecycle `31177298978` passed; schema 3 went live/100 with 12 authenticated artifacts, byte-identical public manifests (`147b2bd7788bc33fd68d4e317d235672fb14a8d68db58ad9dd8022ed57980267`), and artifact index `58422dee82ba9c2b0c804190518478a5569af722f3837667efa30b123547d94b` |
| 2026-08-07 | Spark 2 restart and private-route hardening | Non-loopback image API calls now require the Bridge-held bearer key; loopback Open WebUI and watchdog health remain keyless. Docker restart policies, `@reboot`, and the minute watchdog are installed; controlled image-API and SAM service stops recovered automatically, with the final SAM recovery completing in about six seconds |
| 2026-08-08 | 0.3.72 browser trust boundary + production publication | Exact commit `4998f15` passed 259 Bridge tests, 43/43 desktop tests, full CI `31259807209`, and signed/notarized macOS lifecycle `31259808144`; schema 3 went live/100 with 12 authenticated artifacts. Installed Windows auto-updated through the signed tray path, retained byte-identical pairing/configuration files, ran payload 0.3.72, retired both overlay routes with HTTP 410, and served the trusted localhost dashboard with a nonce CSP. |
| 2026-08-08 | 0.3.72 maintained npm publication | Published the scrubbed 115-file package as `@empir3hq/empir3-bridge@0.3.72`; registry metadata, ownership, repository, integrity, both command shims, entrypoint syntax, a clean isolated install, and a zero-vulnerability production audit were verified. The former `@empir3/empir3-bridge@0.3.22` package remains historical and unsupported. |
